...
object UvWithBloom {
  def main(args: Array[String]): Unit = {
    ...
    uvStream.print()
    env.execute("uv_with_bloom_job")
  }
}

// 触发器，每来一条数据，直接出发窗口计算并清空操作状态
...
class UvCountWithBloom() extends ProcessWindowFunction[(String, Long), UvCount, String, TimeWindow]{
  ...
  // 本来是收集齐所有数据、窗口出发计算的时候才会调用；现在每来一条数据都调用一次
  override def process(key: String, context: Context, elements: Iterable[(String, Long)], out: Collector[UvCount]): Unit = {
    // 先定义Redis中存储位图的key
    val storedBitMapKey = context.window.getEnd.toString

    // 另外将当前窗口的uv count值，作为状态保存到Redis例，用一个叫做uvcount的hash表来保（windowEnd, count）
    ...
    val isExisit = jedis.getbit(storedBitMapKey, offset)
    if(!isExisit){
      // 如果不存在，那么位图对应的位置置为1，并且将count值加1
